...
Use context cost to choose the rollover point: weigh remaining token pressure and useful cache/context against the cost of reloading the compact Mission Brief, active packet and exact HEAD. A compaction followed by a substantially different Route domain is a strong rollover signal; a coherent nearby repair is a reason to stay. Do not set a fixed checkpoint count or assume cache telemetry that the runtime does not expose. The 2026-09-26 user supplement requests periodic fresh Sol ownership for token efficiency while retaining DEC-71 Mission continuity.
